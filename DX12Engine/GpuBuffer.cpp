...
void GpuBuffer::Create( const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const void* initialData )
{
    ...
    ASSERT_SUCCEEDED( 
        gCore->mDevice->CreateCommittedResource( &HeapProps, D3D12_HEAP_FLAG_NONE,
    ...
    const void* initialData)
{
    ...
    ASSERT_SUCCEEDED(gCore->mDevice->CreatePlacedResource(pBackingHeap, HeapOffset, &ResourceDesc, m_UsageState, nullptr, MY_IID_PPV_ARGS(&m_pResource)));
...
D3D12_CPU_DESCRIPTOR_HANDLE GpuBuffer::CreateConstantBufferView(uint32_t Offset, uint32_t Size) const
{
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE hCBV = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    gCore->mDevice->CreateConstantBufferView(&CBVDesc, hCBV);
    return hCBV;
}
...
void ByteAddressBuffer::CreateDerivedViews(void)
{
    ...
    if (m_SRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_SRV = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    gCore->mDevice->CreateShaderResourceView(m_pResource.Get(), &SRVDesc, m_SRV);
    ...
    if (m_UAV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_UAV = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    gCore->mDevice->CreateUnorderedAccessView( m_pResource.Get(), nullptr, &UAVDesc, m_UAV );
}
...
void StructuredBuffer::CreateDerivedViews(void)
{
    ...
    if (m_UAV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_UAV = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    gCore->mDevice->CreateUnorderedAccessView(m_pResource.Get(), m_CounterBuffer.GetResource(), &UAVDesc, m_UAV);
}
