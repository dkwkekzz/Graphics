...
void ColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
{
    ...
    {
        m_RTVHandle = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        m_SRVHandle = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }

    ID3D12Resource* Resource = m_pResource.Get();

    // Create the render target view
    gCore->mDevice->CreateRenderTargetView(Resource, &RTVDesc, m_RTVHandle);

    // Create the shader resource view
    gCore->mDevice->CreateShaderResourceView(Resource, &SRVDesc, m_SRVHandle);
    ...
    for (uint32_t i = 0; i < NumMips; ++i)
    {
        if (m_UAVHandle[i].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_UAVHandle[i] = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

        gCore->mDevice->CreateUnorderedAccessView(Resource, nullptr, &UAVDesc, m_UAVHandle[i]);
...
{
    AssociateWithResource(gCore->mDevice, Name, BaseResource, D3D12_RESOURCE_STATE_PRESENT);

    //m_UAVHandle[0] = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    //gCore->mDevice->CreateUnorderedAccessView(m_pResource.Get(), nullptr, nullptr, m_UAVHandle[0]);

    m_RTVHandle = gCore->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    gCore->mDevice->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle);
}
    ...
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
{
    ...
    CreateTextureResource(gCore->mDevice, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(gCore->mDevice, Format, 1, NumMips);
}
    ...
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem )
{
    ...
    CreateTextureResource(gCore->mDevice, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(gCore->mDevice, Format, ArrayCount, 1);
}
...
//
//    Context.SetRootSignature(gCore->mGenerateMipsRS);
//
...
//            Context.SetPipelineState(gCore->mGenerateMipsGammaPSO[NonPowerOfTwo]);
//        else
//            Context.SetPipelineState(gCore->mGenerateMipsLinearPSO[NonPowerOfTwo]);
//
